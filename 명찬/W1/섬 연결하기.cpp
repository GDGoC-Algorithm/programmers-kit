...
int solution(int n, vector<vector<int>> costs) {
    ...
    check_list.resize(n, -1); //각 노드의 루프 탐지
    costs_table.resize(costs.size(), 0); //해당 다리 건설됬는지 여부
    ...
    for (int i = 0; i < n - 1; i++)
    {
        for (int j = 0; j < costs.size(); j++)
        {
            ...
            if (costs_table[j] == 0)
            {
                if (check_loop(costs[j][0]) != check_loop(costs[j][1])) // 루트부분을 둘다 찾아줘야함. 이전에 1개만 찾아서 오답.
                {
